...
public class Parser {
    ...
    public ParsedCommand getParsedCommand(String text) {
        ...
        PairA commandAndText = getDelimitedCommandFromText(trimText);
        ...
        if (result.getCommand() == Command.NONE) {
            ...
            if (emojiContainsInText.size() > 0) result.setCommand(Command.EMOJI);
        }
        return result;
    }
    ...
    private PairA getDelimitedCommandFromText(String trimText) {
        PairA commandText;

        if (trimText.contains(" ")) {
            int indexOfSpace = trimText.indexOf(" ");
            commandText = new PairA(trimText.substring(0, indexOfSpace), trimText.substring(indexOfSpace + 1));
        } else commandText = new PairA(trimText, "");
        return commandText;
    }
